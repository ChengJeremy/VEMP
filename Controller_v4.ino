...
void loop() {
  ...
  int Blue = 6; //Emitter on transistor, set to zero/GND
  int Button = A3; //Data acquistion initiaion button
  int Yellow = A2; //Striker internal sensor
  int FSR1 = A1; //FSR1 - LEFT 
  int FSR2 = A0; //FSR2 - Right 

  if(analogRead(Button) > 1000){ // && analogRead(Yellow) > 1000  ) { *******
  ...
  //allowing the solenoid to fire
    ...
    for(int i=0; i < S; i++){
      ...
      while(millis() <= set1) //Read signal for T ms after strike
      {
        ...
        Serial.println(val1);  //Printing to serial port
